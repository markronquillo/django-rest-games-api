...
class PlayerTests(APITestCase):
	...
	def test_create_and_retrieve_player(self):
		...
		"""
		url = reverse('player-list')
		new_player_name = 'Mark Joseph'
		new_player_gender = 'M'
		response = self.create_player(name=new_player_name, gender=new_player_gender)
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
	...
	def test_create_duplicated_player(self):
		...
		new_player_gender = 'M'
		response = self.create_player(name=new_player_name, gender=new_player_gender)
		another_player_name = "Mark Joseph"
		another_player_gender = 'M'
		response2 = self.create_player(name=another_player_name, gender=another_player_gender)
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
		self.assertEqual(Player.objects.count(), 1)
		self.assertEqual(
			Player.objects.get().name,
	...
	def test_filter_player_by_name_and_gender(self):
		"""
		Ensure we can filter by name and/or gender
		"""
		url = reverse('player-list')
		new_player_name = 'Mark Joseph'
		new_player_gender = 'M'
		...
		another_player_name = "Mark Joseph2"
		another_player_gender = 'M'
		self.create_player(name=another_player_name, gender=another_player_gender)

		another_player_name = "Antimom"
		another_player_gender = 'F'
		self.create_player(name=another_player_name, gender=another_player_gender)

		data = {'name': 'Mark Joseph'}
		search_url = "{0}?{1}".format(reverse('player-list'), urlencode(data))
		...
		data = {'name': 'Mark Joseph', 'gender': 'M'}
		search_url = "{0}?{1}".format(reverse('player-list'), urlencode(data))
		response = self.client.get(search_url, format='json')
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(len(response.data['results']), 1)
...
class GameTests(APITestCase):
	def create_game(self, game_category, name, release_date, played):
